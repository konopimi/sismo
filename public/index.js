...
// ================================================================
//  BACKLOG PROMOTION MODAL
// ================================================================
// Map backlog intent to suggested target types
const INTENT_TO_TARGET_TYPES = {
  report_need: ["person", "pet", "building"],
  report_emergency: ["person", "pet", "building"],
  donate_offer: ["donacion"],
  request_help: ["necesidad", "person", "pet"],
  ...
  find_supplier: ["anuncio", "necesidad"],
  ask_about_point: ["anuncio"],
  share_info: ["anuncio"],
  report_issue: ["anuncio", "building"],
  mental_health: ["anuncio"],
  fraud_warning: ["anuncio"],
  gratitude: ["anuncio"],
  welcome_member: ["anuncio"],
  request_confirmation: ["anuncio"],
  unknown: ["anuncio", "person", "pet", "building", "donacion", "necesidad", "logistica", "colaborador"],
};

// Target type metadata for the promotion modal
const TARGET_TYPE_META = {
  person: { label: "🫂 Persona desaparecida", api: "disappeared", emoji: "🫂" },
  pet: { label: "🐕 Mascota perdida", api: "pets", emoji: "🐕" },
  building: { label: "🏢 Edificio / Lugar", api: "buildings", emoji: "🏢" },
  donacion: { label: "🎁 Donación", api: "donaciones", emoji: "🎁" },
  necesidad: { label: "🆘 Necesidad", api: "necesidades", emoji: "🆘" },
  logistica: { label: "🚚 Logística", api: "logistica", emoji: "🚚" },
  colaborador: { label: "🤝 Colaborador", api: "collaborators", emoji: "🤝" },
  anuncio: { label: "📢 Anuncio general", api: "anuncios", emoji: "📢" },
};

let currentPromoteBacklogId = null;

function openPromoteModal(backlogId) {
  const item = backlogData.find((b) => String(b.id) === String(backlogId));
  if (!item) return;
  currentPromoteBacklogId = backlogId;

  const suggestedTypes = INTENT_TO_TARGET_TYPES[item.intent] || ["anuncio"];
  const typeOptions = suggestedTypes
    .map((t) => `<option value="${t}">${TARGET_TYPE_META[t]?.label || t}</option>`)
    .join("");

  // Pre-fill form from extracted_data / enriched_data
  const extracted = Array.isArray(item.extracted_data) ? item.extracted_data : [];
  const enriched = item.enriched_data ? JSON.parse(item.enriched_data) : null;

  // Helper to find entity value
  const getEntity = (key) => {
    const e = extracted.find((x) => x.entity === key);
    return e ? e.value : "";
  };
  const getEnriched = (key) => (enriched && enriched[key] != null ? enriched[key] : "");

  // Build form fields based on target type (will update dynamically)
  function buildFormFields(targetType) {
    ...
    switch (targetType) {
      case "person":
        return `
          <div class="form-row"><label>Nombre *</label><input type="text" id="promoteName" value="${escapeHtml(getEntity("name") || getEnriched("name") || "")}" required /></div>
          <div class="form-row"><label>Ubicación</label><input type="text" id="promoteLocation" value="${escapeHtml(getEntity("location") || getEnriched("location") || "")}" /></div>
          <div class="form-row"><label>Ciudad</label><input type="text" id="promoteCity" value="${escapeHtml(getEntity("city") || getEnriched("city") || "")}" /></div>
          <div class="form-row"><label>Descripción</label><textarea id="promoteDescription">${escapeHtml(getEntity("description") || getEnriched("notes") || item.raw_text || "")}</textarea></div>
          <div class="form-row"><label>Contacto</label><input type="text" id="promoteContact" value="${escapeHtml(getEntity("contact") || getEnriched("contact") || "")}" /></div>
          <div class="form-row"><label>Lat</label><input type="number" step="any" id="promoteLat" value="${getEnriched("lat") || ""}" /></div>
          <div class="form-row"><label>Lng</label><input type="number" step="any" id="promoteLng" value="${getEnriched("lng") || ""}" /></div>
        `;
      case "pet":
        return `
          <div class="form-row"><label>Nombre *</label><input type="text" id="promoteName" value="${escapeHtml(getEntity("name") || getEnriched("name") || "")}" required /></div>
          <div class="form-row"><label>Raza</label><input type="text" id="promoteBreed" value="${escapeHtml(getEntity("breed") || getEnriched("breed") || "")}" /></div>
          <div class="form-row"><label>Color</label><input type="text" id="promoteColor" value="${escapeHtml(getEntity("color") || getEnriched("color") || "")}" /></div>
          <div class="form-row"><label>Sexo</label><input type="text" id="promoteSex" value="${escapeHtml(getEntity("sex") || getEnriched("sex") || "")}" /></div>
          <div class="form-row"><label>Tamaño</label><input type="text" id="promoteSize" value="${escapeHtml(getEntity("size") || getEnriched("size") || "")}" /></div>
          <div class="form-row"><label>Ubicación</label><input type="text" id="promoteLocation" value="${escapeHtml(getEntity("location") || getEnriched("location") || "")}" /></div>
          <div class="form-row"><label>Ciudad</label><input type="text" id="promoteCity" value="${escapeHtml(getEntity("city") || getEnriched("city") || "")}" /></div>
          <div class="form-row"><label>Descripción</label><textarea id="promoteDescription">${escapeHtml(getEntity("description") || getEnriched("notes") || item.raw_text || "")}</textarea></div>
          <div class="form-row"><label>Contacto nombre</label><input type="text" id="promoteContactName" value="${escapeHtml(getEntity("contact_name") || getEnriched("contact_name") || "")}" /></div>
          <div class="form-row"><label>Contacto teléfono</label><input type="text" id="promoteContactPhone" value="${escapeHtml(getEntity("contact_phone") || getEnriched("contact_phone") || "")}" /></div>
          <div class="form-row"><label>Contacto WhatsApp</label><input type="text" id="promoteContactWhatsApp" value="${escapeHtml(getEntity("contact_whatsapp") || getEnriched("contact_whatsapp") || "")}" /></div>
          <div class="form-row"><label>Contacto email</label><input type="text" id="promoteContactEmail" value="${escapeHtml(getEntity("contact_email") || getEnriched("contact_email") || "")}" /></div>
          <div class="form-row"><label>Lat</label><input type="number" step="any" id="promoteLat" value="${getEnriched("lat") || ""}" /></div>
          <div class="form-row"><label>Lng</label><input type="number" step="any" id="promoteLng" value="${getEnriched("lng") || ""}" /></div>
        `;
      case "building":
        return `
          <div class="form-row"><label>Nombre / Dirección *</label><input type="text" id="promoteName" value="${escapeHtml(getEntity("name") || getEnriched("name") || "")}" required /></div>
          <div class="form-row"><label>Ubicación</label><input type="text" id="promoteLocation" value="${escapeHtml(getEntity("location") || getEnriched("location") || "")}" /></div>
          <div class="form-row"><label>Ciudad</label><input type="text" id="promoteCity" value="${escapeHtml(getEntity("city") || getEnriched("city") || "")}" /></div>
          <div class="form-row"><label>Estado</label><select id="promoteStatus"><option value="seguro">Seguro</option><option value="danado">Dañado</option><option value="colapsado">Colapsado</option><option value="acopio">Acopio</option></select></div>
          <div class="form-row"><label>Descripción</label><textarea id="promoteDescription">${escapeHtml(getEntity("description") || getEnriched("notes") || item.raw_text || "")}</textarea></div>
          <div class="form-row"><label>Privado</label><input type="checkbox" id="promotePrivate" /></div>
          <div class="form-row"><label>Lat</label><input type="number" step="any" id="promoteLat" value="${getEnriched("lat") || ""}" /></div>
          <div class="form-row"><label>Lng</label><input type="number" step="any" id="promoteLng" value="${getEnriched("lng") || ""}" /></div>
        `;
      case "donacion":
        return `
          <div class="form-row"><label>Tipo *</label><select id="promoteItemType"><option value="comida">Comida</option><option value="ropa">Ropa</option><option value="insumos">Insumos</option><option value="medicinas">Medicinas</option><option value="transporte">Transporte</option><option value="otro">Otro</option></select></div>
          <div class="form-row"><label>Cantidad</label><input type="text" id="promoteQuantity" value="${escapeHtml(getEntity("quantity") || getEnriched("quantity") || "")}" /></div>
          <div class="form-row"><label>Descripción *</label><textarea id="promoteDescription" required>${escapeHtml(getEntity("description") || getEnriched("notes") || item.raw_text || "")}</textarea></div>
          <div class="form-row"><label>Ubicación</label><input type="text" id="promoteLocation" value="${escapeHtml(getEntity("location") || getEnriched("location") || "")}" /></div>
          <div class="form-row"><label>Contacto</label><input type="text" id="promoteContact" value="${escapeHtml(getEntity("contact") || getEnriched("contact") || "")}" /></div>
        `;
      case "necesidad":
        return `
          <div class="form-row"><label>Tipo *</label><select id="promoteItemType"><option value="comida">Comida</option><option value="agua">Agua</option><option value="medicinas">Medicinas</option><option value="ropa">Ropa</option><option value="herramientas">Herramientas</option><option value="transporte">Transporte</option><option value="voluntarios">Voluntarios</option><option value="otro">Otro</option></select></div>
          <div class="form-row"><label>Cantidad</label><input type="text" id="promoteQuantity" value="${escapeHtml(getEntity("quantity") || getEnriched("quantity") || "")}" /></div>
          <div class="form-row"><label>Punto de entrega</label><input type="text" id="promotePointName" value="${escapeHtml(getEntity("point_name") || getEnriched("point_name") || "")}" /></div>
          <div class="form-row"><label>Urgencia</label><select id="promoteUrgency"><option value="baja">Baja</option><option value="media">Media</option><option value="alta">Alta</option><option value="critica">Crítica</option></select></div>
          <div class="form-row"><label>Descripción</label><textarea id="promoteDescription">${escapeHtml(getEntity("description") || getEnriched("notes") || item.raw_text || "")}</textarea></div>
          <div class="form-row"><label>Contacto</label><input type="text" id="promoteContact" value="${escapeHtml(getEntity("contact") || getEnriched("contact") || "")}" /></div>
        `;
      case "logistica":
        return `
          <div class="form-row"><label>Tipo de tarea *</label><select id="promoteTaskType"><option value="entrega">Entrega</option><option value="recoleccion">Recolección</option><option value="transporte">Transporte</option><option value="otro">Otro</option></select></div>
          <div class="form-row"><label>Origen</label><input type="text" id="promoteOrigin" value="${escapeHtml(getEntity("origin") || getEnriched("origin") || "")}" /></div>
          <div class="form-row"><label>Destino</label><input type="text" id="promoteDestination" value="${escapeHtml(getEntity("destination") || getEnriched("destination") || "")}" /></div>
          <div class="form-row"><label>Descripción</label><textarea id="promoteDescription">${escapeHtml(getEntity("description") || getEnriched("notes") || item.raw_text || "")}</textarea></div>
          <div class="form-row"><label>Contacto</label><input type="text" id="promoteContact" value="${escapeHtml(getEntity("contact") || getEnriched("contact") || "")}" /></div>
        `;
      case "colaborador":
        return `
          <div class="form-row"><label>Nombre *</label><input type="text" id="promoteName" value="${escapeHtml(getEntity("name") || getEnriched("name") || "")}" required /></div>
          <div class="form-row"><label>Habilidad / Cómo ayuda *</label><input type="text" id="promoteSkill" value="${escapeHtml(getEntity("skill") || getEnriched("skill") || "")}" required /></div>
          <div class="form-row"><label>Contacto</label><input type="text" id="promoteContact" value="${escapeHtml(getEntity("contact") || getEnriched("contact") || "")}" /></div>
          <div class="form-row"><label>Ciudad</label><input type="text" id="promoteCity" value="${escapeHtml(getEntity("city") || getEnriched("city") || "")}" /></div>
        `;
      case "anuncio":
        return `
          <div class="form-row"><label>Título</label><input type="text" id="promoteTitle" value="${escapeHtml(getEntity("title") || getEnriched("title") || item.intent || "")}" /></div>
          <div class="form-row"><label>Texto *</label><textarea id="promoteText" required>${escapeHtml(item.raw_text || "")}</textarea></div>
          <div class="form-row"><label>Imagen URL</label><input type="text" id="promoteImage" value="${escapeHtml(getEntity("image") || getEnriched("image") || "")}" /></div>
        `;
      default:
        return "";
    }
  ...
  // Initial render with first suggested type
  const initialType = suggestedTypes[0];
  modalTitle.textContent = `Promover: ${item.intent}`;
  modalStatus.innerHTML = `<span class="status-tag backlog-${item.status}">${BACKLOG_STATUS_LABEL[item.status]}</span>`;
  modalMeta.innerHTML = `<span>🧠 Intent: ${item.intent}</span><span>👤 ${escapeHtml(item.creator_name || item.creator_matrix_id || "—")}</span>`;
  modalBody.innerHTML = `
    <div style="margin-bottom:12px;">
      <label style="display:block; margin-bottom:4px;"><b>Tipo de reporte a crear:</b></label>
      <select id="promoteTargetType" style="width:100%; padding:8px; border-radius:8px; border:1px solid #333; background:#1a1d24; color:#eaeaea;">
        ${typeOptions}
      </select>
    </div>
    <div id="promoteFormFields" style="display:flex;flex-direction:column;gap:10px;">
      ${buildFormFields(initialType)}
    </div>
    <details style="margin-top:12px;"><summary>🔍 Datos originales (raw)</summary><pre style="max-height:200px;overflow:auto;">${escapeHtml(item.raw_text || "")}</pre></details>
  `;
  modalActions.innerHTML = `
    <button class="btn-small" onclick="executePromotion()">🚀 Promover</button>
    <button class="btn-small btn-delete" onclick="closeModal();">Cancelar</button>
  `;

  // Update form when target type changes
  document.getElementById("promoteTargetType").addEventListener("change", (e) => {
    document.getElementById("promoteFormFields").innerHTML = buildFormFields(e.target.value);
  });
...
async function executePromotion() {
  const targetType = document.getElementById("promoteTargetType").value;
  const meta = TARGET_TYPE_META[targetType];
  if (!meta) return alert("Tipo de destino inválido");

  // Collect form data
  const getVal = (id) => {
    const el = document.getElementById(id);
    return el ? (el.type === "checkbox" ? el.checked ? 1 : 0 : el.value) : "";
  };

  let payload = {};
  switch (targetType) {
    case "person":
      payload = { name: getVal("promoteName"), location: getVal("promoteLocation"), city: getVal("promoteCity"), description: getVal("promoteDescription"), contact: getVal("promoteContact"), lat: getVal("promoteLat") || null, lng: getVal("promoteLng") || null };
      break;
    case "pet":
      payload = { name: getVal("promoteName"), breed: getVal("promoteBreed"), color: getVal("promoteColor"), sex: getVal("promoteSex"), size: getVal("promoteSize"), location: getVal("promoteLocation"), city: getVal("promoteCity"), description: getVal("promoteDescription"), contact_name: getVal("promoteContactName"), contact_phone: getVal("promoteContactPhone"), contact_whatsapp: getVal("promoteContactWhatsApp"), contact_email: getVal("promoteContactEmail"), lat: getVal("promoteLat") || null, lng: getVal("promoteLng") || null };
      break;
    case "building":
      payload = { name: getVal("promoteName"), location: getVal("promoteLocation"), city: getVal("promoteCity"), status: getVal("promoteStatus"), description: getVal("promoteDescription"), private: getVal("promotePrivate"), lat: getVal("promoteLat") || null, lng: getVal("promoteLng") || null };
      break;
    case "donacion":
      payload = { item_type: getVal("promoteItemType"), quantity: getVal("promoteQuantity"), description: getVal("promoteDescription"), location: getVal("promoteLocation"), contact: getVal("promoteContact") };
      break;
    case "necesidad":
      payload = { item_type: getVal("promoteItemType"), quantity: getVal("promoteQuantity"), point_name: getVal("promotePointName"), urgency: getVal("promoteUrgency"), description: getVal("promoteDescription"), contact: getVal("promoteContact") };
      break;
    case "logistica":
      payload = { task_type: getVal("promoteTaskType"), origin: getVal("promoteOrigin"), destination: getVal("promoteDestination"), description: getVal("promoteDescription"), contact: getVal("promoteContact") };
      break;
    case "colaborador":
      payload = { name: getVal("promoteName"), skill: getVal("promoteSkill"), contact: getVal("promoteContact"), city: getVal("promoteCity") };
      break;
    case "anuncio":
      payload = { title: getVal("promoteTitle"), text: getVal("promoteText"), image: getVal("promoteImage") };
      break;
  }

  // Validate required fields
  if (targetType === "person" && !payload.name) return alert("Nombre es requerido");
  if (targetType === "pet" && !payload.name) return alert("Nombre es requerido");
  if (targetType === "building" && !payload.name) return alert("Nombre/Dirección es requerido");
  if (targetType === "donacion" && !payload.description) return alert("Descripción es requerida");
  if (targetType === "necesidad" && !payload.description) return alert("Descripción es requerida");
  if (targetType === "colaborador" && (!payload.name || !payload.skill)) return alert("Nombre y habilidad son requeridos");
  if (targetType === "anuncio" && !payload.text) return alert("Texto es requerido");

  try {
    const res = await fetch(`${API_BASE}/${meta.api}`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${getAuthToken()}` },
      body: JSON.stringify(payload),
    });
    ...
    // Mark backlog as accepted
    await fetch(`${API_BASE}/backlog/${currentPromoteBacklogId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${getAuthToken()}` },
    ...
    if (targetType === "person") loadList();
    else if (targetType === "pet") loadListP();
    else if (targetType === "building") loadListB();
    else if (targetType === "donacion") loadListDonaciones();
    else if (targetType === "necesidad") loadListNecesidades();
    else if (targetType === "logistica") loadListLogistica();
    else if (targetType === "colaborador") loadListColab();
    else if (targetType === "anuncio") loadListA();

    alert(`✅ Promovido a ${meta.label}`);
  } catch (e) {
    console.error(e);
